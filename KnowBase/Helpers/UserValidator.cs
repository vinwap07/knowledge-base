...
public class UserRegisterValidator : AbstractValidator<User>
{
    public UserRegisterValidator()
    {
        ...
        RuleFor(u => u.Password)
            .MatchPassword()
            .WithMessage("Пароль должен иметь длину от 8 до 15 символов");
        RuleFor(u => u.Name)
            .NotEmpty()
...
public static class CustomValidators
{
    ...
    {
        return ruleBuilder.SetValidator(new RegularExpressionValidator<T>(@"^[a-zA-Z\d]{8,15}$"));
    }
    
    public static IRuleBuilderOptions<T, string> MatchName<T>(this IRuleBuilder<T, string> ruleBuilder)
    {
        return ruleBuilder.SetValidator(new RegularExpressionValidator<T>(@"^[a-zA-Z0-9]{8,15}$"));
    }
